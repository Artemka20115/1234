...
function loadFileUpload(){
	...
	var
		...
		input   = document.getElementById("uploadInput1"),
		
		// the submit button
		submit  = document.getElementById("uploadInput2"),
	...
	noswfupload.event.add(submit, "click", function(e){
		...
		if(wrap.files.length){
			...
			noswfupload.text(wrap.dom.info, "Не выбран не один файл");
	...
	wrap.fileType = "*.jpg, *.jpeg, *.png, *.gif, *.bmp";
	...
	wrap.onload = function(rpe, xhr){
		...
		noswfupload.text(this.dom.info, "Загрузка файла завершена");
		...
		specialObjectImageUploaded(UID);
	};
...
var $ = {
        ...
        $.event.add(wrap.dom.input, "change", function(){
            ...
            for(var max = false, input = wrap.dom.input.cloneNode(true), i = 0, files = $.files(wrap.dom.input); i < files.length; i++){
                ...
                if(wrap.fileType && -1 === wrap.fileType.indexOf("*." + ext)){
					...
					if (wrap.files.length>0) 
					{
						while ( wrap.dom.ul.childNodes.length >= 1 )
						{
							wrap.dom.ul.removeChild( wrap.dom.ul.firstChild );       
						} 
					}
    ...
    lang:{
        ...
        removeFile:"Нажмите 2 раза на имя изображения для удаления",
        
        // after a file has been removed
        removedFile:"Файл {fileName} удален",
        
        // file over maxSize parameter
        errorSize:"ВНИМАНИЕ - Максимальный размер файла {maxSize}. Файл {fileName} имеет {fileSize}",
        
        // file without a valid type
        errorType:"Внимание - Файл {fileName} имеет не правильное расширение. Поддерживаемые виды изображений: {fileType}"
    },
        ...
        if(xhr.upload || xhr.sendAsBinary)
            sendFile = function(handler, maxSize){
                ...
                if(handler.file.getAsBinary){
                    ...
                    xhr.setRequestHeader("TargetWidth", specialObjectImageWidth);
					xhr.setRequestHeader("TargetHeight", specialObjectImageHeight);
					UID=uniqid("img_",true);
					xhr.setRequestHeader("FilenameUID", UID);					
    ...
    wrap:function(input, maxSize){
        ...
        return  $.events({
            ...
            // something like {onload:function(){alert("OK")},onerror:function(){("Error")}, etc ...}
