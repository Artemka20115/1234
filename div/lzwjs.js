...
function InStream(bytestream, bitcount)
{
	...
	this.ReadBit = function()
	{
	    ...
	    var tmp = this.bytestream[this.offset];
	    this.offset++;
	    return tmp&1;
	}
...
function LZWDecompressor(instream)
{
	...
	this.DecompressDictionary = function()
	{
		...
		this.revhashtable[0]='0';
		this.revhashtable[1]='1';
	...
	this.decompress = function(data, bitcount)
	{
	   ...
	   while ((k = this.input.Read(numBits)) != null)
	   {
		  ...
		  for (var i=0; i<entry.length; i++)
			output[output.length] = entry.charAt(i);
...
function compressLZW(inArr)
{
	compressResult="";
	var output = new OutStream();
	var compressor = new LZWCompressor(output);
	compressor.compress(inArr);
	return compressResult;
}

function deCompressLZW(inArr)
{
	var instream = new InStream(inArr, inArr.length);
